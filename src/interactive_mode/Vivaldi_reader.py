...
def memcpy_p2p_send(task, dest_rank, data, halo_size):
	...
	comm.send(rank,					dest=dest_rank,	tag=5)
	comm.send("memcpy_p2p_recv",		dest=dest_rank,	tag=5)
	comm.send(task,					dest=dest_rank,	tag=57)
	comm.send(halo_size,				dest=dest_rank,	tag=57)
