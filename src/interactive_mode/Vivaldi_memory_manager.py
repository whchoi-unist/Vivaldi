...
def synchronize():
	...
	if synchronize_flag == True and function_list == [] and memcpy_tasks == {} and work_list == {} and data_list == [] and memcpy_tasks_to_harddisk == {}:
		...
		for dest in idle_list:
			...
			comm.send("synchronize",	dest=dest,	tag=5)
			comm.recv(source=dest, tag=999)
		comm.send(True, dest=0, tag=999)
		synchronize_flag = False
def disconnect():
...
def memcpy_p2p(source, dest, task):
	comm.send(rank,             	dest=source,		tag=5)
	comm.send("memcpy_p2p_send",	dest=source,		tag=5)
...
def Free(u,ss,sp,data_halo):
	...
	for dest in free_list:
		...
		if dest != None:
			...
			comm.send("free",    dest=dest,	tag=tag)
			tag = int("%d%d"%(tag,8))
			comm.send(u,         dest=dest,	tag=tag)
			comm.send(ss,        dest=dest,	tag=tag)
			comm.send(sp,        dest=dest,	tag=tag)
			comm.send(data_halo, dest=dest,	tag=tag)
...
while flag != "finish":
	...
	if flag == "function":
		...
		for dest in computing_unit_list:
			...
			comm.send(Vivaldi_code,   dest=dest, tag=tag)
	elif flag == "merge":
		st = time.time()
...
#		flag_times[flag] += time.time()-st
		...
		comm.send(data_package,				 dest=dest,	   tag=501)
		...
		comm.send(rank, dest=2, tag=5)
		comm.send("prepare_out_of_core_save", dest=2, tag=5)
